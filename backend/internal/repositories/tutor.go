...
func (r *TutorRepository) Get(ctx context.Context, filter model.TutorFilter) ([]model.Tutor, model.Metadata, error) {
	...
	db := r.db.Read.WithContext(ctx).Model(&model.Tutor{}).Joins("LEFT JOIN users User ON User.id = tutors.user_id").Preload("User")
	...
	if len(filter.IDs) > 0 {
		db = db.Where("tutors.id IN (?)", filter.IDs)
	}

	if filter.UserID != uuid.Nil {
		db = db.Where("tutors.user_id = ?", filter.UserID)
	}
	...
	if !filter.CreatedAtFrom.IsZero() {
		db = db.Where("tutors.created_at >= ?", filter.CreatedAtFrom)
	}

	if !filter.CreatedAtTo.IsZero() {
		db = db.Where("tutors.created_at <= ?", filter.CreatedAtTo)
	}

	if filter.DeletedIsNull.Valid {
		if filter.DeletedIsNull.Bool {
			db = db.Where("tutors.deleted_at IS NULL")
		} else {
			db = db.Where("tutors.deleted_at IS NOT NULL")
		}
	}

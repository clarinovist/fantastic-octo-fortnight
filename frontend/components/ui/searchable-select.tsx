...
export function SearchableSelect<T extends { id: string | number }>({
  ...
  const { data, error, size, setSize, isValidating, isLoading } = useSWRInfinite<ApiResponse<T>>(
    getKey,
    fetcher,
    {
  ...
  const isReachingEnd =
    ...
    (data &&
      ...
      data[data.length - 1].metadata.page >=
        Math.ceil(data[data.length - 1].metadata.total / data[data.length - 1].metadata.pageSize))
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (!observerRef.current || isSearching || !forceShowDropdown || disabled) return
  ...
  const shouldShowDropdown = forceShowDropdown && showDropdown && hasInteracted && !disabled
    ...
    return (
      ...
      >
        ...
        <span className="text-gray-700 flex-1">
          ...
          {secondaryText && <span className="text-gray-500 text-sm block">{secondaryText}</span>}
        </span>
  ...
  return (
    <div
      className={`flex items-center gap-2 flex-1 relative ${disabled ? "opacity-100" : ""} ${className}`}
    >
      {icon && iconPosition === "left" && icon}
      <Input
        ref={inputRef}
        placeholder={placeholder}
        className={`placeholder:text-main-lighten-25 disabled:opacity-100 placeholder:font-bold border-none shadow-none bg-transparent font-bold focus-visible:ring-0 px-0 ${disabled ? "cursor-not-allowed" : ""}`}
        value={value}
        ...
        >
          ...
          {items.length > 0 && (
            <>
              ...
              {isLoadingMore && !isReachingEnd && (
                <div className="px-6 py-3 text-center text-gray-500 text-sm">Loading more...</div>
              )}
          ...
          {isEmpty && !isLoading && (
            <div className="px-6 py-3 text-gray-500 text-sm">
              {value && !showAllItems ? `No items found for "${value}"` : "No items available"}
            </div>
          )}

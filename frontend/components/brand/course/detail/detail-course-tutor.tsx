  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        className={
          variant === "desktop"
            ? "w-[160px] h-[160px] flex-shrink-0 relative"
      ...
      <div className="flex-1">
        <h2 className={`font-bold text-center ${variant === "mobile" ? "text-2xl" : "text-xl"}`}>
          {course.tutor.name}
        </h2>
        <div className="flex flex-wrap gap-1 mt-2">
          {course.relatedCourses.map((tag, index) => (
            <Badge
              key={index}
              variant="secondary"
              className={`bg-gray-100 text-[#7000FE] font-bold ${variant === "mobile" ? "" : "text-xs"}`}
            >
              {tag}
            </Badge>
          ))}

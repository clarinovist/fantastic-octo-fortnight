...
interface TopTutorsProps {
  topBookedData: Promise<BaseResponse<TopBookedTutor[]>>;
  topViewedData: Promise<BaseResponse<TopViewedTutor[]>>;
}
...
export function TopTutors({ topBookedData, topViewedData }: TopTutorsProps) {
  ...
  return (
    <Card className="border-slate-200/60 bg-white/80 p-8 shadow-sm backdrop-blur-sm">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-slate-900">Top Tutors</h2>
        <p className="text-sm text-slate-500">
          Most popular tutors by bookings and views
        </p>
      </div>
            ...
            <div className="flex h-[200px] items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-2 text-sm text-slate-400">
                  No tutors have been booked yet
                </p>
              </div>
            </div>
          ) : (
            topBooked.map((tutor, index) => (
              <Card
                key={tutor.tutorId}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative h-12 w-12 overflow-hidden rounded-full">
                    <Image
                      src={tutor.photoProfile || "/placeholder-avatar.png"}
                      alt={tutor.tutorName}
                      fill
          ...
          {isLoading ? (
            ...
            <div className="flex h-[200px] items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-2 text-sm text-slate-400">
                  No tutors have been viewed yet
                </p>
              </div>
            </div>
          ) : (
            topViewed.map((tutor, index) => (
              <Card
                key={tutor.tutorId}
                className="border-slate-200/60 bg-white p-4 transition-all hover:shadow-md"
              >

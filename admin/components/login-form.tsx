    ...
    <form action={formAction} className={cn("flex flex-col gap-5", className)} {...props}>
      <div className="space-y-2">
        ...
        <Input
          ...
          name="email"
          type="email"
          placeholder="admin@lesprivate.com"
          required
          disabled={isPending}
          className="h-12 rounded-xl border-input bg-background px-4 text-base focus-visible:ring-violet-600/20 focus-visible:border-violet-600 transition-all"
        />
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between px-1">
          ...
          <a
            ...
            className="text-sm font-semibold text-violet-600 hover:text-violet-700 hover:underline underline-offset-4 transition-colors"
          >
            Forgot password?
          </a>
        </div>
        <div className="relative">
          <Input
            ...
            required
            disabled={isPending}
            className="h-12 rounded-xl border-input bg-background px-4 pr-12 text-base focus-visible:ring-violet-600/20 focus-visible:border-violet-600 transition-all"
          />
          <button
            type="button"
            onClick={() => setShowPassword(!showPassword)}
            className="absolute right-0 top-0 h-full px-4 text-muted-foreground hover:text-violet-600 transition-colors flex items-center justify-center"
          >
            {showPassword ? (
      ...
      <div className="flex items-center px-1">
        <label className="flex items-center gap-3 cursor-pointer group">
          <input
            type="checkbox"
            className="h-5 w-5 rounded-md border-input text-violet-600 focus:ring-violet-600/30 transition-all"
          />
      ...
      <Button
        type="submit"
        disabled={isPending}
        className="h-12 w-full rounded-xl text-base font-bold bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-600/20 transition-all mt-2"
      >
      ...
      <Button
        ...
        variant="outline"
        className="h-12 w-full rounded-xl text-sm font-bold border-border bg-background hover:bg-accent text-foreground transition-all"
      >
        <svg
          aria-hidden="true"

...
**Key Patterns:**
* **Error Wrapping:** Use `fmt.Errorf("context: %w", err)` for lower-level errors.
* **Service Errors:** Use `services.Error()` to wrap shared errors into service-specific error types that include HTTP codes and user-friendly messages.
* **Logging:** Always use `logger.ErrorCtx(ctx)` before returning a 500 equivalent.
